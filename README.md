...
A. 수정
   1) 시간 동기화
    ...
    - 시간 동기화 명령 시 이전 동기화 명령으로 이미 동기화를 마쳤거나 부팅 시 자동으로 동기화가 마친 경우 동기화 명령 시에도 timeout error가 뜨는 것을 확인
      
    - 시간 동기화가 이미 되어있다면 명령 대신 동기화만 확인하여 Sync Done으로 변경
    

   2) Waypoint, Rivz btn
      
    - 로봇 2대 중 1대만 종료 했을 경우 Waypoint, Rivz btn이 비활성화 되던 오류 수정
      
    - 마지막 1대까지 종료 했을시에만 공통 프로세스 종료 + 버튼 OFF + 비활성화


B. 확인된 버그
  1) hit_count값이 1씩 증가가 아닌 2씩 증가
  2) Water 값 오류
  3) -
