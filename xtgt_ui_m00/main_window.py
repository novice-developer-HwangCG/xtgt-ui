...
from PyQt5.QtWidgets import QMainWindow, QLabel, QPushButton, QProgressBar, QMessageBox, QLineEdit, QPlainTextEdit, QComboBox
...
"""
    ...
    xtgt1_cur_spd_val   현재 속도 cmd_vel
    xtgt1_cur_dir_val   현재 방향 (yaw)
    -- [재수정] xtgt1_stat_val → common_sur_val      gps (window에서 항상 하던 것을 상태표시로) 사용안함
    xtgt1_dist_val      누적 이동 거리
    xtgt1_rssi_val      신호값
    -- 0313 xtgt1_rtk_val       gps
    xtgt1_tcp_val       통신
    -- 0225 xtgt1_time_sync_val     시간
    xtgt1_water_val     침수센서값 (pico)

    xtgt1_bat_bar       배터리 adc
    xtgt1_stop_btn      비상정지

    xtgt1_mnq_img_lbl   마네킹 사진 (추후 이미지 넣기 위해 라벨말고 frame으로 변경 예정 우선은 라벨로)
    xtgt1_hit_time_lbl  탄 감지 시간
    xtgt1_hit_num_lbl   누적 탄 감지
    ++ 0310 xtgt1_hit_area_lbl      hit 구역

    [수정] xtgt1_hit_save_btn → common_hit_save_btn  탄 감지 log save
    xtgt1_hit_rst_btn   누적 탄 감지 초기화
    ++ 0226 xtgt1_key_btn    메뉴얼 키보드
    ++ 0226 xtgt1_mnq_ctrl_btn 마네킹 수동/자동
    ++ 0226 xtgt1_mnq_up_btn  (수동)마네킹 올리기
    ++ 0226 xtgt1_mnq_down_btn  (수동)마네킹 내리기
    ++ 0305 xtgt1_stick_btn     메뉴얼 조이스틱 on
    ++ 0303 xtgt1_reboot_btn    sbc 재부팅
    ++ 0303 xtgt1_shutdown_btn  sbc 종료
    ++ 0310 xtgt1_stick_off_btn 메뉴얼 조이스틱 off

    xtgt1_lidar_btn    라이다 on/off
    xtgt1_wheel_arm_btn             모터 알람 해제
    xtgt1_time_set_btn              시간 동기화
    -- 0225 xtgt1_debug_btn                 터미널창?
    [수정] common_wps_btn      waypoint 실행    (+++추가)
    [수정] common_rviz_btn       Rviz 실행
    [수정] common_mapviz_btn     Mapbiz 실행    0304 update 버튼은 남겨두고 비활성화 시키기 사용 하지 않음

    [수정] xtgt1_run_btn → xtgt1_auto_run_btn               자율 주행 bringup 실행
    [수정] xtgt1_exit_btn → xtgt1_auto_exit_btn              자율 주행 bringup 종료

    0311 수정
    -- 0311 xtgt1_normal_select         엔코더 선택 (radio button)
    -- 0311 xtgt1_gps_select            gps 선택 (radio button)
    * 라디오 선택 버튼 제거 gps로만 자율주행 사용
    ++ xtgt1_ch1_ignore_btn     1 채널 무시
    ++ xtgt1_ch2_ignore_btn     2 채널 무시
    ++ xtgt1_ch3_ignore_btn     3 채널 무시
    ++ critical_val         치명상
    ++ non_critical_val     비치명상
    ++ hit_apply_btn       피격(치명, 비치명) 횟수 적용
    ++ xtgt1_manual_run_btn               수동 bringup 실행
    ++ xtgt1_manual_exit_btn              수동 bringup 종료
    [추가 했다 삭제함] common_rtk_base_btn  ?

    0312 수정
    ++ logbox_text      우측 하단 log 출력용

    0417 수정
    ++ spd_cb       속도 단위 선택  (default 값 m/s, combo box 리스트 m/s, km/h, mph)
    ++ spd_apply_btn        속도 단위 적용
"""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, ui_path: str):
        ...
        self.robot_ip = {"xtgt1": "192.168.0.111", "xtgt2": "192.168.0.112"}
        self.robot_pw = {"xtgt1": "0000", "xtgt2": "0000"}
        self.robot_user = {"xtgt1": "m1", "xtgt2": "m2"}     # 로봇의 topic name은 xtgt1, xtgt2, 실제 sbc명은 m1, m2 xtgt1 = m1, xtgt2 = m2
        self.ssh_port = 22
        ...
        self.lidar_log_lines = []
        self.lidar_log_dir = Path.home() / "Desktop" / "logs" / "Lidar_logs"
        self.lidar_log_started_at = datetime.now().strftime("%y%m%d_%H%M")
        ...
        self.hit_count_raw = {r: -1 for r in self.robot_num}
        self.mnq_reset_timers = {}
        self.critical_hit = 1
        self.non_critical_hit = 1
        self.ch1 = 0        # 0 채널 사용, 1 무시
        self.ch2 = 0
        self.ch3 = 0
        self.selected_spd_unit = "m/s"
        ...
        self.spd_cb = self.findChild(QComboBox, "spd_cb")
        if self.spd_cb is None:
            raise RuntimeError("ComboBox not found : object name='spd_cb'")
        ...
        v = QIntValidator(1, 9, self)
        for w, default in [(self.critical_edit, "1"), (self.non_critical_edit, "1")]:
        ...
        for robot in self.robot_num:
            ...
            timer.timeout.connect(lambda r=robot: self.change_mnq(r, 0))
            self.mnq_reset_timers[robot] = timer
    ...
    def connect_signals_and_init(self):
        ...
        self.hit_apply_btn = self.common["hit_apply_btn"]
        spd_apply_btn = self.common["spd_apply_btn"]

        self.spd_cb.clear()
        self.spd_cb.addItems(["m/s", "km/h", "mph"])
        self.spd_cb.setCurrentText("m/s")   # default

        waypoint_btn.setCheckable(True)
        rviz_btn.setCheckable(True)
        hit_save_btn.setCheckable(False)
        self.hit_apply_btn.setCheckable(False)
        ...
        self.spd_cb.currentTextChanged.connect(self.spd_unit_set)
        spd_apply_btn.clicked.connect(self.spd_unit_apply)
    ...
    # ==================== speed unit ====================
    def spd_unit_set(self, unit: str):
        # 콤보박스에서 단위 선택될 때 호출
        self.selected_spd_unit = unit
        #print(f"spd set: {unit}")

    def spd_unit_apply(self):
        # 단위 적용 버튼 눌렀을 때 호출
        unit = self.spd_cb.currentText()
        #print(f"spd apply {unit}")
    ...
    def lidar_set(self, robot: str, checked: bool):
        ...
        if checked:
            self.lidar_log_lines.append(f"{robot} lidar off - {now_str}")
        else:
            self.lidar_log_lines.append(f"{robot} lidar on - {now_str}")

    # 라이다 on,off 시 메모장 기록함수 YY:MM:DD:HH:MM으로 언제 on / off를 하였는지 log 기록 함수
    def save_lidar_mode(self):
        if not self.lidar_log_lines:
            return

        try:
            self.lidar_log_dir.mkdir(parents=True, exist_ok=True)

            file_name = f"lidar_on_off_log_{self.lidar_log_started_at}.txt"
            file_path = self.lidar_log_dir / file_name

            with open(file_path, "w", encoding="utf-8") as f:
                f.write("\n".join(self.lidar_log_lines) + "\n")

        except Exception as e:
            QMessageBox.warning(self, "Lidar Log Save Error", str(e))
    ...
    # ==================== common set (waypoint / rviz) ====================
    def common_set_ui(self):
        """로봇 실행 상태(self.running)에 따라 공통(WPS/RViz) 버튼을 제어"""
        any_running = any(self.running.values())

        wps_btn  = self.common["common_wps_btn"]
        rviz_btn = self.common["common_rviz_btn"]

        wps_btn.setEnabled(any_running)
        rviz_btn.setEnabled(any_running)

        if not any_running:
            self.stop_common_processes()

            with QSignalBlocker(wps_btn):
                wps_btn.setChecked(False)
            with QSignalBlocker(rviz_btn):
                rviz_btn.setChecked(False)
    ...
    def on_threshold_changed(self):
        ...
        self.critical_hit = int(c_txt) if c_txt.isdigit() else 1
        self.non_critical_hit = int(n_txt) if n_txt.isdigit() else 1
    ...
    def change_mnq(self, robot: str, mnq: int):
        ...
        if mnq in (1, 2, 3):
            ...
            self.labels[area_key].setText(str(mnq))
            self.record_hit_log()
        ...
        if mnq in (1, 2, 3):
            self.mnq_reset_timers[robot].stop()
            self.mnq_reset_timers[robot].start(3000)
    ...
    # ==================== Save log ==================== 
    def save_hit_log(self, out_dir: str = str(Path.home() / "Desktop" / "logs")):  # 기존 out_dir: str = "logs"
    ...
    def save_dist_vals(self, robots=("xtgt1", "xtgt2"), out_dir: str = (Path.home() / "temp" / "dis_logs")): # 기존 out_dir: str = "logs"
        os.makedirs(out_dir, exist_ok=True)
        ts = datetime.now().strftime("%y%m%d_%H%M%S")
...
class TimeSyncWorker(QObject):
    ...
    def wait_master_selected(self, ssh, retries=30, delay_sec=0.5):
    ...
    def run(self):
        ...
        try:
            ...
            # 먼저 "이미 마스터 ^*"인지 빠르게 확인
            ok_master, master_detail, sources_full = self.wait_master_selected(ssh, retries=3, delay_sec=0.3)
            if ok_master:
                # 이미 붙어있으면 명령 실행 없이 성공 처리
            ...
            # 실행 후 ^* 확인
            ok_master, master_detail, sources_full = self.wait_master_selected(ssh, retries=30, delay_sec=0.5)
            if not ok_master:
                err = f"After sync, master not selected\n{master_detail}\n\n[SOURCES]\n{sources_full}"
                if out1:
                    err = f"[CMD OUT]\n{out1}\n\n" + err
